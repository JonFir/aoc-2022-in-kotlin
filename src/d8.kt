    ...
    fun part1(input: List<String>): Int = input.flatMap { it.trim().split("").drop(1).dropLast(1).map { it.toInt() } }
            .chunked(input.size * input.size).map { matrix ->
                matrix.foldIndexed(0) { index, acc, item ->
                    if (
                            (index / input.size == 0 || index / input.size == input.size - 1 || index % input.size == 0 || index % input.size == input.size - 1)
                            || ((index / input.size * input.size until index).map { matrix[it] }.any { it >= item  }.not()
